/*
    ...
    written by Jens M?nig
    jens@moenig.org

    Copyright (C) 2016 by Jens M?nig
...
*/
    ...
    canvasProto._isRetinaEnabled = false;
...
CursorMorph.prototype.initializeClipboardHandler = function () {
	...
	*/
	this.clipboardHandler.onkeydown= function (event) {
		console.log('me-keydown.......');
		myself.processKeyDown(event);
		/*
		var sval = myself.target.selection();
		var clval = myself.clipboardHandler.value;\
		var sindex = myself.target.selectionStartSlot();
		if(sval != ''){
			myself.clipboardHandler.value = clval.replace(sval,"");
			console.log('me-keydown clvale=',myself.clipboardHandler.value);
			myself.target.clearSelection();
			myself.gotoSlot(sindex);
			myself.gotoPos(myself.target.slotPosition(sindex+2));
		}*/
    ...
    this.clipboardHandler.addEventListener(
        ...
        function (event) {
            ...
            myself.processKeyDown(event);
            this.value = myself.target.selection();
			console.log('me-keydown...select/this value=',this.value);
            this.select();
    ...
    this.clipboardHandler.addEventListener(
        ...
        function (event) {
			...
			var sval = myself.target.selection();
			var clval = myself.clipboardHandler.value;
			console.log('me-input...select value=',sval);
			myself.clipboardHandler.value = clval.replace(sval,"");
			myself.target.text = myself.clipboardHandler.value;
			myself.gotoSlot(myself.clipboardHandler.selectionStart+1);            
            myself.target.drawNew();//console.log('me-input myval=',myself.clipboardHandler.value);
			myself.target.changed();//console.log('me-input myselect=',myself.clipboardHandler.selectionStart);
			if(sval == ''){
				myself.gotoSlot(myself.clipboardHandler.selectionStart);
			}else{
				var sindex = myself.target.selectionStartSlot();
                console.log('me-input...select value not sindex = ',sindex);
                myself.target.clearSelection();
                myself.gotoSlot(sindex+1);
                //set cursor..........
				myself.gotoPos(myself.target.slotPosition(sindex+2));
				/**/
			}
...
CursorMorph.prototype.gotoSlot = function (slot) {
    var length = this.target.text.length,
        ...
        right,
        left;
    //console.log('me-gotoslot pos=',pos);
...
StringMorph.prototype.init = function (
    ...
    // additional properties:
    this.text = text || ((text === '') ? '' : 'StringMorph-1');
...
StringMorph.prototype.mouseDownLeft = function (pos) {
    ...
    //console.log('me-mouseDownLeft pos=',pos);
    if (this.isEditable) {
        this.clearSelection();
    } else {
        this.escalateEvent('mouseDownLeft', pos);
...
StringMorph.prototype.mouseClickLeft = function (pos) {
    var cursor;
   //console.log('me-mouseClickLeft pos=',pos);

    if (this.isEditable) {
        if (!this.currentlySelecting) {
            //console.log('me-mouseClickLeft ----currentlySelecting---');
